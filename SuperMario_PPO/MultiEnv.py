...
num_actions = Common_constants.num_actions


class SubProcessEnv( object ):
    def env_process(self, conn):
        # env = make_train_0()
        # env = make_atari(name)        
...
class EnvActor( object ):
    ...
    def step_env(self, policy_net, value_net, t):
        ...
        self.delta.append(
            self.rew.get( t ) + (1 - AUX.indicator( done_t )) * gamma * self.val.get( t + 1 ) - self.val.get( t ) )
